...
class Controller_OAuth_Twitter extends Controller_OAuth_Base {
	...
	{
		if (Auth::instance()->logged_in_oauth('twitter'))
		{
			echo 'You signed in via Twitter. Yay!';
		}
		else
		{
			$this->template->content = HTML::anchor($this->request->uri(array('action' => 'signin')), 'Sign in with Twitter');
		}
	}
